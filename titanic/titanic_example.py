...
# https://www.kaggle.com/samratp/beginner-tutorial-using-votingclassifier-82-27
...
#    missing_age_data = test_data_orig[test_data_orig['Age'].isnull()]
#    missing_age_data = test_data_orig[test_data_orig['Age'].notnull()]
...
def fill_missing_age(missing_age_train, missing_age_test):
    ...
    gbm_reg_grid = model_selection.GridSearchCV(
                              gbm_reg, gbm_reg_param_grid, 
                              cv = 10, n_jobs = 3, verbose = 1, 
...
#    missing_age_test['Age'] = missing_age_test[['Age_GB', 'Age_LRF']].mode(axis = 1)
    missing_age_test['Age'] = 0.5*missing_age_test['Age_GB'] + 0.5*missing_age_test['Age_LRF']
    print("missing_age_test['Age'][:4] is ", missing_age_test['Age'][:4])
...
def get_top_n_features(titanic_train_data_X, titanic_train_data_y, top_n_features):
    ...
    rf_grid = model_selection.GridSearchCV(rf_est, rf_param_grid, n_jobs = 3, cv = 10, verbose = 1)
    ...
    ada_grid = model_selection.GridSearchCV(ada_est, ada_param_grid, n_jobs = 3, cv = 10, verbose = 1)
    ...
    et_grid = model_selection.GridSearchCV(et_est, et_param_grid, n_jobs = 3, cv = 10, verbose = 1)
...
if __name__=='__main__':
    ...
    print('train_data_orig.info() is ', train_data_orig.info())
    print('train_data_orig.describe() is ', train_data_orig.describe())
    print('train_data_orig.head() is ', train_data_orig.head())

    test_data_orig['Survived'] = 0
    combined_train_test = train_data_orig.append(test_data_orig)
    print('combined_train_test.shape is ', combined_train_test.shape)
#    print(combined_train_test.info())
#    print(combined_train_test.describe())
    
    
    print('here111', 
          combined_train_test.groupby(['Survived', 'Embarked'])['Survived'].count())
    print('here222',
          combined_train_test['PassengerId'].groupby(by = combined_train_test['Embarked']).count().sort_values(ascending = False))
    print('here333',
          combined_train_test['Fare'].groupby(by = combined_train_test['Embarked']).mean().sort_values(ascending = False))
    ...
    print('combined_train_test columns111 is ', combined_train_test.shape)
    ...
    print('combined_train_test columns222 is ', combined_train_test.shape)
    
    combined_train_test['Title'] = combined_train_test['Name'].str.extract('.+,(.+)').str.extract('^(.+?)\.').str.strip()
    print("combined_train_test['Title'] is ", combined_train_test['Title'])
    ...
    print('show show is ', dict.fromkeys(["Master"], "Master"))
    ...
    print('title_dummies_df columns is ', title_dummies_df.columns)
    ...
    if (combined_train_test['Fare'].isnull().sum() != 0):
        print('isnull sum not equal 0', combined_train_test.groupby('Pclass').transform('mean').shape)
#        print('isnull sum not equal 0')
    ...
    print("combined_train_test['Group_Ticket'] is ", combined_train_test['Group_Ticket'])
    combined_train_test['Fare'] = combined_train_test['Fare']/combined_train_test['Group_Ticket']
    combined_train_test.drop(['Group_Ticket'], axis = 1, inplace = True)
    print('ticket count is', 
          combined_train_test['Ticket'].groupby(by = combined_train_test['Ticket']).count().sort_values(ascending = False)[:10])
    ...
    print("combined_train_test['Age_Null'] is ", combined_train_test['Age_Null'])
    ...
    scale_age_fare = StandardScaler().fit(combined_train_test[['Age', 'Fare']])
    ...
    print('select important feature started...')
    ...
    print('titanic_train_data_X.shape is ', titanic_train_data_X.shape)
    titanic_test_data_X = titanic_test_data_X[features_top_n]
    print('titanic_test_data_X.shape is ', titanic_test_data_X.shape)
    
    print('start training model')
    
    rf_est = RandomForestClassifier(n_estimators = 3000, criterion = 'gini', max_features = 'sqrt', max_depth = 3, min_samples_split = 4, min_samples_leaf = 2, n_jobs = 2, random_state = 42, verbose = 1)
    gbm_est = GradientBoostingClassifier(n_estimators = 3500, learning_rate = 0.0008, min_samples_split = 3, min_samples_leaf = 2, max_features ='sqrt', max_depth = 3,  random_state = 42, verbose = 1)
    et_est = ExtraTreesClassifier(n_estimators = 3000, max_features = 'sqrt', max_depth = 35,  n_jobs = 3, random_state = 42, verbose = 1)
...
#                               param_grid, cv=5, verbose=1, n_jobs=3)
#    grid_search.fit(titanic_train_data_X, titanic_train_data_y)
#    test_score = grid_search.score(titanic_train_data_X, titanic_train_data_y)
#    titanic_test_data_X['Survived'] = grid_search.predict(titanic_test_data_X)
#    end_t = time.time()
#    print('trainging model cost time: {} seconds'.format(end_t - begin_t))
...
#    begin_t = time.time()
#    df_concated = pd.concat([titanic_train_data_X, titanic_test_data_X])
#    scaler = StandardScaler().fit(df_concated)
#    x_train_scaled = scaler.transform(titanic_train_data_X)
#    x_test_scaled = scaler.transform(titanic_test_data_X)    
#    grid_search = GridSearchCV(SVC(), param_grid, cv=5, n_jobs=2)
#    grid_search.fit(x_train_scaled, titanic_train_data_y)
#    test_score = grid_search.score(x_train_scaled, titanic_train_data_y)
#    titanic_test_data_X['Survived'] = grid_search.predict(x_test_scaled)
#    end_t = time.time()    
#    print('trainging model cost time: {} seconds'.format(end_t - begin_t))
#    
...
#    print('combined_train_test columns is ', combined_train_test.columns)

#    pdd_1 = pd.DataFrame({'A': [3, 4, 5, 10],
#                          'B': [-3, -8, -8, -9],
#                          'B1': [90, 10, 70, 80],
#                          'C': ['a', 'b', 'c', 'd'],
#                          'D': ['111', '222', '444', '333'],
#                          'Name': ['John, Mr. Tu.', 'Kun, Mr Wu', 'Same, Mr. Zhou', 'Bill, Mr. Gates'] })
#    print(pdd_1[['A', 'B', 'B1']].corr(method='kendall'))
#    pdd_1['Last_Name'] = pdd_1['Name'].str.split("\.").str[1].str.strip()
#    print('pdd_1 is ', pdd_1)
#    itle_dummies_df = pd.get_dummies(pdd_1, prefix='OKOK', 
#                                     prefix_sep='&', columns=['D'],
#                                     dummy_na=True)
#    le_fare = LabelEncoder()
#    le_fare.fit(np.array(['111', '222', '333', '444']))
#    pdd_1['Name_Length_Category'] = le_fare.transform(pdd_1['D'])
#    print('pdd_1 is ', pdd_1)
###    pdd_2 = pd.DataFrame({'A': [3, 4, 5, 10],
###                        'B': [9, 10, 7, 8],
###                        'c': ['a', 'b', 'c', 'd'] })
#    pdd_2 = pd.DataFrame({'A': [3, 4, 5, 10],
#                        'B': [9, 10, 7, 8],
#                         'D': ['a', 'b', 'c', 'd']})
#    concated_dataframe = pd.concat([pdd_1, pdd_2], axis=1)
##    pdd_new = pdd_1.append(pdd_2)
##    pdd_new = pdd_new[['A', 'B', 'c', 'D']]
#    print('concated_dataframe is ', list(concated_dataframe.columns))
#    print('concated_dataframe is ', concated_dataframe['A'])
